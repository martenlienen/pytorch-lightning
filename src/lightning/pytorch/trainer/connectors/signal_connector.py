...
class _SignalHandlerCallback(Callback):
    ...
    def notify_connector(self, *args, **kwargs):
...
class _SignalConnector:
    ...
    The connector only stores the reception of signals in flags and then processes them
    at the next possible opportunity in the current loop. This minimizes the amount of
    code running in signal handlers, because file IO in signal handlers can crash the
    process. This also guarantees that we are not checkpointing in the middle of a
    backward pass or similar.
    """
